...
export class InventoryPage extends BasePage {
  ...
  async addToCart(itemName: string | string[]): Promise<Item[]> {
    return await testStep(`Adding '${itemName.toString()}' to cart`, async () => {
      ...
      for (const name of itemNameArray) {
        ...
        const currentItem: Item = {
          name: name,
          description: await itemToChose.locator('[data-test="inventory-item-desc"]').innerText(),
          price: await itemToChose.locator('[data-test="inventory-item-price"]').innerText()
        };
  ...
  async getCartCount(): Promise<number> {
    return await testStep(`Getting cart count`, async () => {
